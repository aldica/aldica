...
| aldica.core.storage.systemInitialSize | `41943040` (40 MiB) | Initial size (in bytes) of the data region reserved for internal Ignite data structures / management of the Ignite data grid |
| aldica.core.storage.systemMaxSize | `104857600` (100 MiB) | Maximum size (in bytes) of the data region reserved for internal Ignite data structures / management of the Ignite data grid |
...
| aldica.core.storage.defaultStorageRegion.maxSize | `17179869184` (16 GiB) | Maximum size (in bytes) of the primary data region used to back all Ignite-based caches unless individual caches have been configured to use dedicated data regions - if the `swapPath` is configured as `null`, this amount of memory must be available (in addition to Java heap, other Java memory, and the size of any other non-swap storage regions) otherwise an out-of-memory-error may eventually occur) |
| aldica.core.storage.defaultStorageRegion.swapPath | `${java.io.tmpdir}/aldica/defaultDataRegionSwap` | Path to a file system directory in which the primary data region will swap if the available physical memory is not sufficient to handle the size of the data region - a textual value of `null` may be used in property overrides to explicitly disable swap-support for the default region and keep its data exclusively in memory |
| aldica.core.storage.region._&lt;name&gt;_.initialSize |  | Initial size (in bytes) of a dynamic, custom data region (identified by the _name_ fragment in the configuration property); this property **cannot** be provided via JAVA\_OPTS _-D_ parameters unless as an override to a value contained in any `alfresco-global.properties` file |
| aldica.core.storage.region._&lt;name&gt;_.maxSize |  | Maximum size (in bytes) of a dynamic, custom data region (identified by the _name_ fragment in the configuration property); this property **cannot** be provided via JAVA\_OPTS _-D_ parameters unless as an override to a value contained in any `alfresco-global.properties` file; if the `swapPath` is configured as `null`, this amount of memory must be available (in addition to Java heap, other Java memory, and the size of any other non-swap storage regions) otherwise an out-of-memory-error may eventually occur) |
| aldica.core.storage.region._&lt;name&gt;_.swapPath |  | Path to a file system directory in which the dynamic, custom data region (identified by the _name_ fragment in the configuration property) will swap if the available physical memory is not sufficient to handle the size of the data region; this property **cannot** be provided via JAVA\_OPTS _-D_ parameters unless as an override to a value contained in any `alfresco-global.properties` file; a textual value of `null` may be used in property overrides to explicitly disable swap-support for the default region and keep its data exclusively in memory |
...
| aldica.core.binary.optimisation.useIdsWhenPossible | `${aldica.core.binary.optimisation.useIdsWhenReasonable}` | global enablement flag for using dynamic value substitution for all types of complex entities that can be resolved via secondary caches, even other complex value caches |
...
| aldica.core.binary.optimisation.txnValueHolder.enabled | `true` | enablement flag for optimisations applied to `TransactionalCache$ValueHolder` instances |
| aldica.core.binary.optimisation.txnValueHolder.useRawSerial | `${aldica.core.binary.optimisation.useRawSerial}` | enablement flag for using raw serial form for `TransactionalCache$ValueHolder` instances |
...
| aldica.core.segmentationPolicy | `NOOP` | Policy to apply when detecting an Ignite grid segmentation - may be `NOOP`, `STOP`, `RESTART_JVM` - the default `NOOP` disconnects the node from the current Ignite grid |
...
| aldica.webSessionCache.instanceName | `${aldica.core.name}` | The name of the data grid to use for instantiating the web session cache |
...
| heap.maxItems | maxItems | Maximum number of on-heap stored cache entries that are allowed before eviction of on-heap data is triggered - defaults to `0` as "not configured" - the Alfresco-equivalent property is only used if  _aldica.caches.ignoreDefaultEvictionConfiguration_ is set to `false` |
| heap.eviction-policy | eviction-policy | Policy to use for the eviction of on-heap data - defaults to `NONE` - the Alfresco-equivalent property is only used if  _aldica.caches.ignoreDefaultEvictionConfiguration_ is set to `false` |
...
| heap.eviction-percentage | eviction-percentage (until Alfresco 5.2) | Percentage of on-heap cache entries to evict in a batch when eviction of on-heap data is triggered by exceeding the _heap.maxItems_ limit - defaults to `0` - the Alfresco-equivalent property is only used if  _aldica.caches.ignoreDefaultEvictionConfiguration_ is set to `false` |
| timeToLiveSeconds | timeToLiveSeconds | The time-to-live (in s) for an individual cache entry after it has been created or updated - defaults to `0` - the Alfresco-equivalent property is only used if  _aldica.caches.ignoreDefaultEvictionConfiguration_ is set to `false` |
| maxIdleSeconds | maxIdleSeconds | The time-to-live (in s) for an individual cache entry after it has last been accessed in the cache - defaults to `0` - the Alfresco-equivalent property is only used if  _aldica.caches.ignoreDefaultEvictionConfiguration_ is set to `false` |
...
| forceInvalidateOnPut |   | Flag to specify if a cache put operation in an invalidating cache should always trigger an invalidation message to other data grid members, even if no effective change has occurred (no replacement of data, e.g. only a simple load-from-db operation) - defaults to `true` for consistency with default Alfresco cache behaviour |
...
The following caches come with explicit overrides to the default cache configuration by this module:

- `immutableEntitySharedCache`
    - `ignite.cache.type=replicated`
    - `ignite.heap.maxItems=5000`
    - `ignite.heap.eviction-policy=${cache.immutableEntitySharedCache.eviction-policy}`
- `ticketsCache` and `usernameToTicketIdCache` as replicated caches
- various caches with `ignite.forceInvalidateOnPut=false`
    - `immutableEntitySharedCache`
    - `node.rootNodesSharedCache`
    - `messagesSharedCache`
    - `loadedResourceBundlesSharedCache`
    - `resourceBundleBaseNamesSharedCache`
    - `propertyClassCache`
    - `authoritySharedCache`
    - `authorityToChildAuthoritySharedCache`
    - `zoneToAuthoritySharedCache`
- various caches with `ignite.cache.type=partitioned`
