...
public class NodeVersionKeyBinarySerializerTests extends GridTestsBase
{
    ...
    protected void correctnessImpl(final IgniteConfiguration conf)
    {
        try (Ignite grid = Ignition.start(conf))
        {
            ...
            NodeVersionKey serialisedValue;
            ...
            final String referenceSerialisationType, final double marginFraction)
    {
        ...
        final Supplier<NodeVersionKey> comparisonValueSupplier = () -> {
            final int idx = 1000 + rnJesus.nextInt(100000000);
            final NodeVersionKey value = new NodeVersionKey((long) idx, 1l + rnJesus.nextInt(1000));

            return value;
        };

        super.serialisationEfficiencyComparison(referenceGrid, grid, "NodeVersionKey", referenceSerialisationType, serialisationType,
