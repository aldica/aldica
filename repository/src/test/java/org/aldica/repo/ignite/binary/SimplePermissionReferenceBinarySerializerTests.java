 ...
 * @author Axel Faust
 */
public class SimplePermissionReferenceBinarySerializerTests extends AclGridTestsBase
{

    private static final Logger LOGGER = LoggerFactory.getLogger(SimplePermissionReferenceBinarySerializerTests.class);
            ...
            final boolean serialForm, final String... regionNames)
    {
        ...
        binaryTypeConfigurationForSimplePermissionReference.setTypeName(SimplePermissionReference.class.getName());
        ...
        binaryConfiguration
                .setTypeConfigurations(Arrays.asList(binaryTypeConfigurationForSimplePermissionReference, binaryTypeConfigurationForQName));
    ...
    public void defaultFormEfficiency()
    {
        try (final GenericApplicationContext appContext = createApplicationContext())
        {
            ...
            try
            {
                ...
                final CacheConfiguration<Long, SimplePermissionReference> cacheConfig = new CacheConfiguration<>();
                cacheConfig.setCacheMode(CacheMode.REPLICATED);

                cacheConfig.setName("comparison1");
                cacheConfig.setDataRegionName("comparison1");
                final IgniteCache<Long, SimplePermissionReference> referenceCache1 = referenceGrid.getOrCreateCache(cacheConfig);
                final IgniteCache<Long, SimplePermissionReference> cache1 = defaultGrid.getOrCreateCache(cacheConfig);

                // optimised QName does a lot - 27%
                ...
                final IgniteCache<Long, SimplePermissionReference> referenceCache2 = referenceGrid.getOrCreateCache(cacheConfig);
                final IgniteCache<Long, SimplePermissionReference> cache2 = useQNameIdGrid.getOrCreateCache(cacheConfig);

                // replacing QName with ID - 42%
    ...
    public void rawSerialFormEfficiency()
    {
        try (final GenericApplicationContext appContext = createApplicationContext())
        {
            ...
            try
            {
                ...
                // QName inlining (raw) vs sub-object serialisation (+metadata) makes for a substantial difference - 18%
                this.efficiencyImpl(referenceGrid, defaultGrid, referenceCache1, cache1, "aldica raw serial", "aldica optimised", 0.18);
                ...
                // QName inlining already does a lot - 22%
                this.efficiencyImpl(referenceGrid, useQNameIdGrid, referenceCache2, cache2, "aldica raw serial (ID substitution)",
                        "aldica optimised", 0.22);

                cacheConfig.setName("comparison3");
                cacheConfig.setDataRegionName("comparison3");
                final IgniteCache<Long, SimplePermissionReference> referenceIdCache3 = referenceIdGrid.getOrCreateCache(cacheConfig);
                final IgniteCache<Long, SimplePermissionReference> cache3 = useQNameIdGrid.getOrCreateCache(cacheConfig);

                // only eliminates some metadata overhead - 2%
                this.efficiencyImpl(referenceIdGrid, useQNameIdGrid, referenceIdCache3, cache3, "aldica raw serial (ID substitution)",
                        "aldica optimised (ID substitution)", 0.02);
            }
            finally
            {
                Ignition.stopAll(true);
    ...
    protected void correctnessImpl(final IgniteConfiguration conf)
    {
        ...
        {
            final CacheConfiguration<Long, SimplePermissionReference> cacheConfig = new CacheConfiguration<>();
            cacheConfig.setName("simplePermissionReference");
            cacheConfig.setCacheMode(CacheMode.REPLICATED);
            final IgniteCache<Long, SimplePermissionReference> cache = grid.getOrCreateCache(cacheConfig);

            SimplePermissionReference controlValue;
            SimplePermissionReference cacheValue;

            controlValue = SimplePermissionReference.getPermissionReference(ContentModel.TYPE_CMOBJECT, PermissionService.CONSUMER);
            cache.put(1l, controlValue);

            cacheValue = cache.get(1l);

            // no equals() in SimplePermissionReference, but AbstractPermissionReference
            Assert.assertEquals(controlValue, cacheValue);
            Assert.assertNotSame(controlValue, cacheValue);

            controlValue = SimplePermissionReference.getPermissionReference(QName.createQName("custom.model", "mytype"),
                    "CustomPermission");
    ...
    protected void efficiencyImpl(final Ignite referenceGrid, final Ignite grid,
            final IgniteCache<Long, SimplePermissionReference> referenceCache, final IgniteCache<Long, SimplePermissionReference> cache,
            final String serialisationType, final String referenceSerialisationType, final double marginFraction)
    {
        LOGGER.info(
                "Running SimplePermissionReference serialisation benchmark of 100k instances, comparing {} vs. {} serialisation, expecting relative improvement margin / difference fraction of {}",
                referenceSerialisationType, serialisationType, marginFraction);

        final SecureRandom rnJesus = new SecureRandom();
        for (int idx = 0; idx < 100000; idx++)
        {
            final SimplePermissionReference value = this.randomPermission(rnJesus);
