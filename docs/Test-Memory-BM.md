...
- reconfiguring the `JAVA_OPTS` for the aldica-enabled instance in the `pom.xml` (e.g. by adding `-Daldica.core.binary.optimisation.enabled=false` to disable all but the most basic optimisations or `-Daldica.core.binary.optimisation.useIdsWhenPossible=true` to enable the more aggressive optimisations)
...
| Measure | Alfresco 6.1.2 | aldica (default) | aldica (max opt) | aldica (min opt) |
| :--- | ---: | ---: | ---: | ---: |
