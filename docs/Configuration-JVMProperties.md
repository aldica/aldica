...
These specific settings may not be strictly required for all combinations ACS / Java versions and aldica binary optimisation settings, and have been collected from multiple tests with different configurations to provide a recommendation that ensures aldica works correctly in any of the configurations tested during development.
...
```
-DIGNITE_PERFORMANCE_SUGGESTIONS_DISABLED=true -DIGNITE_QUIET=true -DIGNITE_NO_ASCII=true -DIGNITE_UPDATE_NOTIFIER=false -DIGNITE_JVM_PAUSE_DETECTOR_DISABLED=true -DIGNITE_SKIP_CONFIGURATION_CONSISTENCY_CHECK=true
```
